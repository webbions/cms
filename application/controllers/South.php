...
class South extends CI_Controller {
	...
	public function login($provider = '', $user_group = '')
	{
		// when open directly
		...
		if ($user_group != '' && $provider != '') {
			$socialSess = [ "provider" => $provider, "user_group" => $user_group ];
			// if user group
			$this->session->set_userdata( 'user_group_sess', $socialSess );
		}else{
			// if not user group
		...
		try
		{
			...
			if ($this->hybridauthlib->providerEnabled($provider))
			{
				...
				if ($service->isUserConnected())
				{
					...
					redirect('/user/register_social/'.$provider);
