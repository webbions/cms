...
class Mahana_messaging
{
    public function __construct()
    {
        ...
        $this->ci->load->model('MahanaModel');
    ...
    function get_message($msg_id, $user_id)
    {
        ...
        if ($message = $this->ci->MahanaModel->get_message($msg_id, $user_id))
        {
    ...
    function get_full_thread($thread_id, $user_id, $full_thread = FALSE, $order_by = 'ASC')
    {
        ...
        if ($message = $this->ci->MahanaModel->get_full_thread($thread_id, $user_id, $full_thread, $order_by))
        {
            return $this->_success($message);
        }
    ...
    function get_all_threads($user_id, $full_thread = FALSE, $order_by = 'ASC')
    {
		...
		$message = $this->ci->MahanaModel->get_all_threads($user_id, $full_thread, $order_by);
        if (is_array($message))
        {
            return $this->_success($message);
        }
     ...
     * @return  array
     */
    function get_all_threads_grouped($user_id, $full_thread = FALSE, $order_by = 'ASC',$limit='')
    {
        if (empty($user_id))
        {
            return $this->_invalid_id(MSG_ERR_INVALID_USER_ID);
		...
		$message = $this->ci->MahanaModel->get_all_threads($user_id, $full_thread, $order_by,$limit);
        if (is_array($message))
        {
            $threads = array();
    ...
    function update_message_status($msg_id, $user_id, $status_id )
    {
        ...
        if ($this->ci->MahanaModel->update_message_status($msg_id, $user_id, $status_id))
        {
    ...
    function add_participant($thread_id, $user_id)
    {
        ...
        if ( ! $this->ci->MahanaModel->valid_new_participant($thread_id, $user_id))
        {
            $this->_particpant_error(MSG_ERR_PARTICIPANT_EXISTS);
        }

        if ( ! $this->ci->MahanaModel->application_user($user_id))
        {
            $this->_particpant_error(MSG_ERR_PARTICIPANT_NONSYSTEM);
        }

        if ($this->ci->MahanaModel->add_participant($thread_id, $user_id ))
        {
    ...
    function remove_participant($thread_id, $user_id)
    {
        ...
        if ($this->ci->MahanaModel->remove_participant($thread_id, $user_id))
        {
    ...
    function send_new_message($sender_id, $recipients, $subject = '', $body = '', $priority = PRIORITY_NORMAL)
    {
        ...
        if ($thread_id = $this->ci->MahanaModel->send_new_message($sender_id, $recipients, $subject, $body, $priority))
        {
    ...
    function reply_to_message($msg_id, $sender_id, $subject = '', $body = '', $priority = PRIORITY_NORMAL)
    {
        ...
        if ($new_msg_id = $this->ci->MahanaModel->reply_to_message($msg_id, $sender_id, $body, $priority))
        {
    ...
    function get_participant_list($thread_id, $sender_id = 0)
    {
        ...
        if ($participants = $this->ci->MahanaModel-> get_participant_list($thread_id, $sender_id))
        {
    ...
    function get_msg_count($user_id, $status_id = MSG_STATUS_UNREAD)
    {
        ...
        if (is_numeric($message = $this->ci->MahanaModel->get_msg_count($user_id, $status_id)))
        {
            return $this->_success($message);
        }
